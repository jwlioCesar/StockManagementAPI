...
public class ProductService {
    ...
    public ProductOutputResponseDto productOutput(ProductOutputRequestDto request){
        Product product = productRepository.findById(request.id()).get();
        product.setQuantity(product.getQuantity()-request.quantity());
        productRepository.save(product);
        return new ProductOutputResponseDto(product.getId(), product.getName(), product.getQuantity(), product.getPrice(), product.getTotalValue());
    }
}